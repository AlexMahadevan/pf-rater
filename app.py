...
st.caption("This experimental generative AI tool compares draft articles against PolitiFact's archive and external fact-checking sources. It identifies relevant jurisprudence, analyzes cross-source consensus and provides structured recommendations to support editorial decisions. It can alternatively be used on standalone claims to see if we've rated anything similar, and suggestions for starting a fact-check.\n\n I used retrieval-augmented generation (RAG) to mostly eliminate hallucinations by tying it to fact-check databases.\n\n There is also an experimental tab that will transcribe audio, and try to identify relevant claims. With a few clicks, it will do the same thing as the text tool. \n\n You can also click a button down there to make the tool search the web for context and advice on breaking claims. \n\n Note: This is a prototype based on 9,000 fact-checks, so answers will not be complete.")
